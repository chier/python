...
class ProxyCheck(threading.Thread):
    ...
    def checkProxy(self):
        ...
        for proxy in self.proxyList:
            ...
            try:
                #req = urllib.request.urlopen("http://www.baidu.com", timeout=self.timeout)
...
def imgurlList(url_home):
    global imgurl_list
    home_page = urllib.request.urlopen(url_home)
    ...
    for url in url_list:
        #print url_home+url
        url_page = urllib.request.urlopen(url_home+url)
...
class getPic(threading.Thread):
    ...
    def downloadimg(self):
        for imgurl in self.imgurl_list:
            ...
            proxyHandler = urllib.request.ProxyHandler({"http" : r'http://%s:%s' %(randomCheckedProxy[0],randomCheckedProxy[1])})
            opener = urllib.request.build_opener(cookies,proxyHandler)
            opener.addheaders = [('User-agent', 'Mozilla/5.0 (Windows NT 6.2; WOW64; rv:22.0) Gecko/20100101 Firefox/22.0')]
            urllib.request.install_opener(opener)
            try:
